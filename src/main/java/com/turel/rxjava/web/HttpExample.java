...
public class HttpExample {
    ...
    public Observable<String> getSite(){
        final HttpAsyncRequestProducer get = HttpAsyncMethods.createGet("http://jsonplaceholder.typicode.com/posts/1");
        return ObservableHttp.createRequest(get, httpClient).toObservable().map(response -> {
            try {
                final InputStream content = response.getResponse().getEntity().getContent();
